...
var PT_doSessionCheck = function (getBack) {
    ...
    PT_doAjax(PT_GetSession, 'get', '', 'application/json', "json", '', function (err, result) {
        if (err) {
            ...
            PT_UserID = result.UserID
            MyAccount();
            if (result.UserID != 0 && result.UserID != null) {
                PT_doAjax(PT_ProjectAccess, 'post', { "userid": PT_UserID, "projectid": PT_ProjectID }, 'application/json', "json", '', function (err, result) {
                    if (err) {
                        getBack(err);
                    }
                    else if (result.ResponseCode == 0) {
                        PT_doAjax(PT_AppAcces, 'post', { "userid": PT_UserID, "appid": PT_AppID }, 'application/json', "json", '', function (err, result) {
                            if (err) {
                                getBack(err);
                            }

                            else if (result.ResponseCode == 1) {
                                // window.location='http://'+ProjectID+'.sblsol.com/apps/'+AppID+"/home/layout";
                                // window.location = "~/apps/" + AppID + "/home/layout";
                                PT_GlobalToken = result.Token;
                                $(".PT_Token").attr("data-token", PT_GlobalToken);
                                getBack(null);
                            }
                            else {
                                console.log(result.ResponseCode);
                                getBack({ err: "Session Failed" });
                        ...
                        getBack({ err: "Session Failed" });
                        window.location = PT_HomeNavigation;
                    }
                });
...
jQuery.fn.extend({
    ...
    ConnateSlider: function (Details, Activity, Chat) {
        ...
        PT_slide += '<div id="PT_Slider_header"><span class="PT_slideTitle">Title</span><span class="PT_closedIcon">X</span></div>';
...
function MyAccount() {
    ...
    PT_doAjax(PT_UserDetailsApi, 'post', { "UserId": PT_UserID }, 'application/json', "json", '', function (err, result) {
        if (err) {
            ...
            ScriptUsername = result.LoginInfo.UserName;
            $(".PT_UserName a").html('');
